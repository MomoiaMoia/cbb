...
__attribute__((aligned(16))) uint8_t sub_0010_arena[640512];
// Fast scratch arena not used for Ethos-U55
//  We will not create it for now and reuse the address of the other arena
// __attribute__((aligned(16))) static uint8_t sub_0010_fast_scratch[640512];
...
int sub_0010_invoke(bool clean_outputs) {
  ...
  // Buffer sub_0010_model with size 1568 and address: 4294967295
  base_addrs[0] = (uint64_t)(uintptr_t)sub_0010_model_data;
  base_addrs_size[0] = sub_0010_model_data_size;
  // Buffer sub_0010_arena with size 640512 and address: 0
  base_addrs[1] = (uint64_t)(uintptr_t) (sub_0010_arena+0);
  base_addrs_size[1] = 640512;

  // Buffer sub_0010_fast_scratch with size 640512 and address: 0
  base_addrs[2] = (uint64_t)(uintptr_t) (sub_0010_arena+0);
  base_addrs_size[2] = 640512;

  // Buffer input_tensor_0 with size 32000 and address: 512
  base_addrs[3] = (uint64_t)(uintptr_t) (sub_0010_arena+512);
  base_addrs_size[3] = 32000;

  // Buffer input_tensor_1 with size 500 and address: 0
  base_addrs[4] = (uint64_t)(uintptr_t) (sub_0010_arena+0);
  base_addrs_size[4] = 500;

  // Buffer output_tensor_0 with size 2000 and address: 512
  if (clean_outputs) {
    memset(sub_0010_arena + 512, 0, 2000);
  }
  base_addrs[5] = (uint64_t)(uintptr_t) (sub_0010_arena+512);
  base_addrs_size[5] = 2000;

  // Buffer output_tensor_1 with size 500 and address: 0
  if (clean_outputs) {
    memset(sub_0010_arena + 0, 0, 500);
  }
  base_addrs[6] = (uint64_t)(uintptr_t) (sub_0010_arena+0);
  base_addrs_size[6] = 500;
