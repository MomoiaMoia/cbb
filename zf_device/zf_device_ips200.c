...
// Hardware Interface
#define IPS200_RST(x)       g_ioport.p_api->pinWrite(&g_ioport_ctrl, BSP_IO_PORT_11_PIN_02, (bsp_io_level_t)x)
#define IPS200_CS(x)        g_ioport.p_api->pinWrite(&g_ioport_ctrl, BSP_IO_PORT_11_PIN_00, (bsp_io_level_t)x)
#define IPS200_DC(x)        g_ioport.p_api->pinWrite(&g_ioport_ctrl, BSP_IO_PORT_11_PIN_01, (bsp_io_level_t)x)

#define IPS200_DELAY_MS(x)  R_BSP_SoftwareDelay(x, BSP_DELAY_UNITS_MILLISECONDS)
#define ips200_spi_init()   g_sci_spi5.p_api->open(g_sci_spi5.p_ctrl, g_sci_spi5.p_cfg);

#define ips200_write_8bit_data_spi(data)            g_sci_spi5.p_api->write(g_sci_spi5.p_ctrl, &data, 1, SPI_BIT_WIDTH_8_BITS)
#define ips200_write_8bit_data_spi_array(ptr, len)  g_sci_spi5.p_api->write(g_sci_spi5.p_ctrl, ptr, len, SPI_BIT_WIDTH_8_BITS)

static void ips200_write_16bit_data_spi (uint16 data)
{
    uint8 temp[2] = {(data >> 8) & 0xFF, (data >> 0) & 0xFF};
    ips200_write_8bit_data_spi_array(temp, 2);
}

static uint8 write_16bit_data_temp[320 * 2];
static void ips200_write_16bit_data_spi_array (uint16 *ptr, uint32 len)
{
    for(uint32 i = 0; len * 2 > i; i += 2)
    {
        write_16bit_data_temp[i + 0] = (*ptr >> 8) & 0xFF;
        write_16bit_data_temp[i + 1] = (*ptr >> 0) & 0xFF;
        ptr ++;
    }
    ips200_write_8bit_data_spi_array(write_16bit_data_temp, len * 2);
}
